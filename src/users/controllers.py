...
class UserController(Controller):
    ...
    async def get_user_list(
        ...
        try:
            ...
            raise HTTPException(status_code=HTTP_400_BAD_REQUEST, detail="bad request") from ex
    ...
    @get("/{id:int}")
    async def get_user(self, user_id: int, user_service: UserRepositoryService) -> User:
        return await get_user(user_id=user_id, user_service=user_service)

    @put("/{id:int}", dto=UpdateUserDTO)
    async def update_user(
            self, request: Request, user_id: int, user_service: UserRepositoryService, data: UpdateUser
    ) -> User:
        if request.user.is_supper_user or request.user.id == user_id:
            return await update_user(user_id=user_id, user_service=user_service, user_data=data)

        raise HTTPException(status_code=HTTP_403_FORBIDDEN, detail="forbidden")

    @delete("/{id:int}")
    async def delete_user(self, request: Request, user_id: int, user_service: UserRepositoryService) -> None:
        if request.user.is_supper_user or request.user.id == user_id:
            await delete_user(user_id=user_id, user_service=user_service)
        else:
            raise HTTPException(status_code=HTTP_403_FORBIDDEN, detail="forbidden")
